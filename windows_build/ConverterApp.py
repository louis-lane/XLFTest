...
# --- NEW: List of protected language names for UI elements ---
# Converted to a lowercase set for fast, case-insensitive lookups
...
def get_target_language(xliff_path):
    try:
        ...
        # We look for the 'file' element which contains the target-language attribute
...
# --- NEW FEATURE: Function to apply DeepL translations (STRICT MATCHING) ---
def apply_deepl_translations(root_path):
    """Reads DeepL output files and applies them to the master Excel files using strict filename matching."""
    ...
    for master_file in master_files:
        ...
        # If master is 'pt-BR-master.xlsx', we extract 'pt-BR' exactly.
        base_lang_code = master_file.name.replace("-master.xlsx", "")
        
        matching_deepl_file = None

        # We look for a DeepL file that starts with the EXACT code.
        ...
        if not matching_deepl_file:
            errors.append(f"No matching DeepL file found for master file: {master_file.name} (Expected {base_lang_code}.xlsx)")
            continue

        try:
            # Read translations, skipping the header row
            ...
            # Construct sheet name using the STRICT code
            ...
            # REMOVED: master_df['status'] = 'Machine Translated'
            # The status column is now left untouched.
...
def process_standard_files(root_path, translated_dir):
    ...
    if not repo_path.exists():
        return # Silently exit if no repository folder exists
...
# --- Core Logic (MODIFIED for Strict Matching) ---
def export_to_excel_with_glossary(root_path, glossary_path=None):
    ...
    # GROUP BY EXACT LANGUAGE CODE (e.g. 'zh-CN', 'zh-HK', 'pt-BR')
    for lang_code, lang_df in master_df.groupby('language'):
        try:
            ...
            with pd.ExcelWriter(master_workbook_path, engine='openpyxl') as writer:
                ...
                deduplicated = deduplicated[['source', 'target', 'count', 'locations', 'status', 'add_to_glossary']]
...
def import_and_reconstruct_with_glossary(root_path, glossary_path=None):
    ...
    # 1. Update Glossary (Optional)
    try:
        new_glossary_entries = []
        for master_file in master_files:
            # STRICT: Extract language code without splitting
    ...
    # 2. Build Translation Map (Keyed by STRICT language code)
    translation_map = {}
    for master_file in master_files:
        try:
            # STRICT: Use full code as key (e.g. 'zh-CN')
            lang_code = master_file.name.replace("-master.xlsx", "")
            ...
            for _, row in df.iterrows():
                translation_map[lang_code][row['source'].strip()] = str(row['target'])
        except Exception as e:
    ...
    for xliff_file in original_files:
        try:
            # STRICT: Get exact target from XML (e.g. 'zh-CN')
            lang_code_xml = get_target_language(xliff_file)
            
            # Lookup in map
            lang_specific_map = translation_map.get(lang_code_xml, {})
            
            # Fallback: Check for case-insensitive match if exact match fails
            ...
            # Sort into folders based on the XML language code
...
def update_xliff_from_map(original_path, lang_specific_map):
    ...
    for trans_unit in tree.xpath('//xliff:trans-unit', namespaces=ns):
        ...
        if source_element is not None and source_element.text:
            normalized_source = source_element.text.strip()
            if normalized_source in lang_specific_map:
                target_element = trans_unit.find('xliff:target', namespaces=ns)
                if target_element is not None:
                    target_element.text = lang_specific_map[normalized_source]
